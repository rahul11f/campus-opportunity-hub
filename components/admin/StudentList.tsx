...
export function StudentList({ initialStudents }: { initialStudents: any[] }) {
  ...
  return (
    <div className="admin-card rounded-xl overflow-visible relative">
      <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] px-5 py-3.5 border-b border-slate-200 text-xs font-semibold text-slate-500 uppercase tracking-wider">
        <div>Student</div>
        <div>Branch</div>
        <div>Batch</div>
        <div>CGPA</div>
      ...
      {!students || students.length === 0 ? (
        ...
        students.map((student) => (
          <div key={student.id} className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] px-5 py-4 border-b border-slate-100 items-center hover:bg-slate-50 transition-colors relative">
            ...
            <div className="text-sm text-slate-600">{student.branch || '-'}</div>
            <div className="text-sm text-slate-600">{student.batch || '-'}</div>
            <div className="text-sm text-slate-600">{student.cgpa || '-'}</div>
            ...
            <div className="relative">
              ...
              {openDropdown === student.user_id && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-48 rounded-xl bg-white border border-slate-200 shadow-2xl overflow-hidden z-50">
                    <button 
                    ...
                    <button 
                      ...
                      className="w-full text-left px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 flex items-center gap-2 border-t border-slate-100"
                    >
